...
```powershell
az group create -n fkubernetes9  -l eastus2 # Create a resource group fkubernetes9
first
...
az group delete -n fkubernetes9 # Delete the resource group - always delete the Kubernetes service 

```
* A resource group named MC_fkubernetes9_fkubernetes9_eastus2 will be created containing all resources (vm, disk, load balancer).
...
```powershell
az aks get-credentials --resource-group fkubernetes9 --name fkubernetes9 # Switch to cluster
```
...
az aks browse --resource-group fkubernetes9 --name fkubernetes9 # Start web server dashboard and open in browser
az aks browse -n fkubernetes9 -g fkubernetes9  # open dashboard # Start web server dashboard and open in browser
```
...
az aks scale --resource-group fkubernetes9 -n fkubernetes9 --agent-count 3
```
...
```powershell
C:\> kubectl config use-context fkubernetes9 # Switch to cluster
