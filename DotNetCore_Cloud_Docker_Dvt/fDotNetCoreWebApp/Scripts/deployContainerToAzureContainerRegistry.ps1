...
param(
    ...
    [string]$action = "instantiate", # build, push, instantiate, deleteInstance    
...
function GetContainerImageName() {
    return (GetCurrentDirName).toLower()
}
...
Write-Host "Build project $(GetProjectName), version:$(GetProjectVersion), ContainerName:$(GetContainerImageName)" -ForegroundColor DarkYellow

$imageTag = GetContainerImageName
$containeInstanceName = (GetContainerImageName) + "instance0"
